...
def dStatedt(t, state, params):
    ...
    dVdt = calc_dVdt(V, A_modulation, g_internal_gating, E_rev_gate_matrix,
                     S_arr, E_rev_syn_matrix, g_syn) + I_ext(t)
...
t_stop = 1000.0 # ms
dt = 0.02     # ms
times_array = np.arange(start= t_start,stop= t_stop, step=dt) # times array for system solution
times_array_stim = np.arange(start= t_start-10*dt,stop= t_stop+10*dt, step=dt)#expanded times for stimulus interpolation
save_utilities.save_text(times_array,"save","./Times_array.txt")
...
I = 40.0
I_ext_arr = np.array([0,
                  I,
                  I/4])# External stimulus (nA)

stim_name = "Constant_40nA"
I_ext_expanded_time_series = (np.multiply(I_ext_arr[None,:],np.ones((times_array_stim.shape))[:,None]))
I_interp_function = scipy.interpolate.interp1d(x=times_array_stim, y=I_ext_expanded_time_series, kind='cubic',axis=0)
save_utilities.save_text(np.vstack((times_array,np.array([I_interp_function(t) for t in times_array]).transpose())).transpose(),
                         "save","custom_stimuli/Constant_Current_I_ext=40.txt")
#--------
# --------------- L63 --------------
max_value = 100
stim_name = f"L63x_max={max_value}nA"
dilation_factor = 0.2
...
timestep = dt #seconds
# times_array_stim_l63x = np.arange(start= t_start-10*dt,stop= float(t_stop)+10*dt, step=timestep)#expanded times for stimulus interpolation

L63_obj = externally_provided_currents.L63_object(scaling_time_factor=scaling_time)
I_L63_x, I_L63_y, I_L63_z = L63_obj.prepare_f(times_array_stim).T
I_L63_x = set_range(I_L63_x, -100, max_value)
I_L63_y = set_range(I_L63_y, -100, max_value)
I_L63_z = set_range(I_L63_z, -100, max_value)
I_ext_expanded_time_series = np.vstack((0*I_L63_x,I_L63_x,0.25*I_L63_x))
I_interp_function = scipy.interpolate.interp1d(x=times_array_stim, y=I_ext_expanded_time_series, kind='cubic',axis=1)
# Data_plot_and_save(I_L63_x, t_arr, max=max_value, name="I_L63_x_time_dilation="+str(dilation_factor))
save_utilities.save_txt_with_makedir(np.vstack((times_array,np.array([I_interp_function(t) for t in times_array]).transpose())).transpose(),
                                     "./custom_stimuli/" + stim_name + "Current_range=" + str(max_value) + ".txt")

plt.figure()
plt.plot([I_interp_function(t) for t in times_array])
plt.xlim((0,1000))
plt.show()
#--------
...
saved_data = np.vstack((sol.t,np.array([I_interp_function(t) for t in times_array]).transpose(),sol.y)).transpose()
print(f"saved_data.shape:{saved_data.shape}")
save_utilities.save_text(saved_data,"save",f"simulated_data/3NN/tIState_stim={stim_name}.txt")
save_utilities.save_fig_with_makedir(figure=fig,
                                     save_location=f"simulated_data/3NN/tIState_stim={stim_name}.png")
plt.close("all")